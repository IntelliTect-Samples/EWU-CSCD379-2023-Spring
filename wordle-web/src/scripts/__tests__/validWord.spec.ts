...
describe('ValidWord', () => {
  ...
  it('No y - doesnt return layer', () => {
    const valid = Word.ValidWords(wordleWords, [new Letter('y', LetterStatus.Wrong)])
    expect(valid.length).toBe(2)
    expect(valid).toEqual(wordleWords.slice(0, 2))
  })

  it('Misplaced y - only return layer', () => {
    const valid = Word.ValidWords(wordleWords, [new Letter('y', LetterStatus.Misplaced)])
    expect(valid.length).toBe(1)
    expect(valid).toEqual(['layer'])
  })

  it('Correct y - only return layer', () => {
    const valid = Word.ValidWords(wordleWords, [new Letter('y', LetterStatus.Correct)])
    expect(valid.length).toBe(1)
    expect(valid).toEqual(['layer'])
  })

  it('Misplaced p - return apple and pears', () => {
    const valid = Word.ValidWords(wordleWords, [new Letter('p', LetterStatus.Misplaced)])
    expect(valid.length).toBe(2)
    expect(valid).toEqual(wordleWords.slice(0, 2))
  })

  it('Correct p - return apple and pears', () => {
    const valid = Word.ValidWords(wordleWords, [new Letter('p', LetterStatus.Correct)])
    expect(valid.length).toBe(2)
    expect(valid).toEqual(wordleWords.slice(0, 2))
  })
})
