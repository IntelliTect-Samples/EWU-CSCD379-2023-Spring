...
export class SignInService {
  ...
  private _canCreateDelete: boolean = false
  ...
  public async signInAsync(username: string, password: string): Promise<boolean> {
    try {
      ...
      const result2 = await Axios.get('Token/CanCreateDelete')
      this._canCreateDelete = result2.data
      console.log(this._canCreateDelete)
      return true
    } catch (err) {
