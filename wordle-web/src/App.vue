          ...
          <template v-slot:append>
            <v-menu>
              <template v-slot:activator="{ props }">
                <v-btn v-bind="props">
                  <Icon icon="ph:gear-fill" color="#155" width="30" height="30" />
                </v-btn>
              </template>
              <v-list>
                <v-btn @click="setInverseTheme"> Inverse Theme </v-btn>
                <v-btn @click="setDarkTheme"> Dark Theme </v-btn>
                <v-btn @click="setCheeseTheme"> Cheese Theme </v-btn>
                <v-btn @click="setPainfulTheme"> Painful Theme </v-btn>
              </v-list>
            </v-menu>
            <v-menu>
              <template v-slot:activator="{ props }">
                <v-btn v-bind="props">
                  <Icon icon="charm:menu-hamburger" color="#155" width="30" height="30" />
                </v-btn>
              </template>
              <v-list>
                <v-list-item>
...
const themeClass = computed(() => {
  if (theme.global.name.value === 'cheese') {
    return 'cheese-theme'
  }
  if (theme.global.name.value === 'painful') {
    return 'painful-theme'
  }
  return ''
})
...
function setCheeseTheme() {
  theme.global.name.value = 'cheese'
}

function setPainfulTheme() {
