...
Axios.get('word/wordlist?pageNumber=0')
    .then( (response) => { words.value = response.data as WordHelper[] })

function nextPage() {
        page++
        if(searchWord.value === ''){
            Axios.get('word/wordlist?pageNumber=' + page)
                .then( (response) => { words.value = response.data as WordHelper[] })
        } else {
            Axios.get('word/wordlist?pageNumber=' + page + '&searchWord=' + searchWord.value)
                .then( (response) => { words.value = response.data as WordHelper[] })
        }
