...
const guess = ref('')
const wordList = ["apple", "goose", "peach", "crypt", "piano", "jello", "zesty", "color", "spice", "water", "sugar", "sauce", "tacos", "theif", "owner", "silly", "pizza", "bread"]
    ...
    if (guessChars [i] !== '_') {
      for (let j = 0; j < 5; j++) {
        if (secretChars [j] === guessChars[i]) {
          ...
          guessChars[i] = '_'
          secretChars[j] = '_'
          console.log('Letter ${i} is partially correct')
          break  
        }
      }
    }
